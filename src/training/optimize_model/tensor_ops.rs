...
use burn::{prelude::Backend, tensor::{Bool, Float, Int, Shape, Tensor}};

use crate::{Bknd, DEVICE};
...
// }
pub fn create_mask<B: Backend>(masked_cols: Tensor<B, 1,Int>,shape:Shape, device: &B::Device) -> Tensor<B,2,Bool> {

    let zeros: Tensor<_, 2, Float> = Tensor::zeros(shape.clone(), device);
    let ones: Tensor<_, 2, Float>  = Tensor::ones(shape.clone(), device);

    let masked_cols = masked_cols.unsqueeze_dim::<2>(1).expand::<2,_>(shape.clone());
    let mask = zeros.scatter(1, masked_cols, ones);
    let mask:Tensor<_,2,Bool> = mask.greater_equal_elem(1.0f32);
 
    mask
}
pub fn mask_negative(mask:Tensor<Bknd,2>) -> Tensor<Bknd,2> {
    let ones: Tensor<_, 2, _> = Tensor::<Bknd, 2, Float>::ones(mask.shape(),&DEVICE);
    mask.neg().add(ones)
}