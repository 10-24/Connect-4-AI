...
impl<B: Backend> Model<B> {
    ...
    pub fn forward_regression(
        ...
        let target_q_vals = target_q_val_builder.build(predicted_q_vals.clone(),&predicted_q_vals.device());
        let loss = MseLoss::new().forward(predicted_q_vals.clone(),target_q_vals.clone(),Reduction::Mean);
...
impl TrainStep<Batch, RegressionOutput<Bknd>> for Model<Bknd> {
    fn step(&self, batch: Batch) -> TrainOutput<RegressionOutput<Bknd>> {
        ...
        let item = self.forward_regression(batch.get_game_states(), batch.create_target_q_val_builder());
